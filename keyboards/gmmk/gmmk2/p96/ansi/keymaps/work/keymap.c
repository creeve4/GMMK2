...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case VOLDN:
        if (record->event.pressed) {
            tap_code(KC_F17);			// Volume Down
        }
        break;

    case VOLUP:
        if (record->event.pressed) {
            tap_code(KC_F18);			// Volume Up
        }
        break;
	}
    return true;
...
// Combos
const uint16_t PROGMEM volume[] = {VOLDN, VOLUP, COMBO_END};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
   ...
   */
[_BL] = LAYOUT(
  KC_ESC,   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,   KC_PSCR,  TD(MLT),  TD(FN),   VOLDN,    VOLUP,
